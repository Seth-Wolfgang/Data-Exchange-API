...
void build_session_id_string(char* buffer, const SessionID session_id, const char* prefix) {
    ...
    int size = snprintf(NULL, 0, "%s,%d,%d,%d,%d,%s", prefix,
    ...
    snprintf(buffer, size + 1, "%s,%d,%d,%d,%d,%s", prefix,
                                                           session_id.source_model_id, // int
                                                           session_id.destination_model_id, // int
                                                           session_id.initiator_id, // int
                                                           session_id.invitee_id, // int 
                                                           session_id.instance_id); // 36 chars
    printf("Constructed Session ID: %s, %d\n", buffer, size);  // Debugging print
}
 ...
 */
SessionID create_session(const char* base_url, int source_model_id, int destination_model_id, 
    ...
    if(curl) {
        ...
        session_id = build_session_id(&chunk);
